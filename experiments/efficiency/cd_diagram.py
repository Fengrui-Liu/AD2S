...
def wilcoxon_holm(alpha=0.05, df_perf=None):
    ...
    friedman_p_value = friedmanchisquare(
        *(
            np.array(df_perf.loc[df_perf["classifier_name"] == c]["value"])
    ...
    for i in range(m - 1):
        ...
        perf_1 = np.array(
            df_perf.loc[df_perf["classifier_name"] == classifier_1]["value"],
            dtype=np.float64,
        )
...
# draw_cd_diagram(df_perf=df_perf, title='value', labels=True)
