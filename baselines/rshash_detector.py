...
from .util import StreamStatistic
from collections import deque


class RShashDetector(BaseDetector):
    def __init__(
        self, decay=0.01, components_num=20, hash_num: int = 10, seed=0, **kwargs
    ):
        ...
        Args:
            ...
            decay (float, optional): Decay ratio. Defaults to 0.2.
            components_num (int, optional): Number of components. Defaults to 50.
            hash_num (int, optional): Number of hash functions. Defaults to 10.
        """
        super().__init__(data_type="multivariate", **kwargs)

        self.decay = decay
        self.data_stats = StreamStatistic(is_global=False,window_len=60)
        ...
        self.effective_s = max(100, 1.0 / (1 - np.power(2, -self.decay)))
        self.f = self.rng.uniform(
    ...
    def fit(self, X: np.ndarray, timestamp: int = None):
        ...
        if self.index == 0:
            self.alpha = [
                self.rng.uniform(low=0, high=self.f[r], size=len(X))
                for r in range(self.components_num)
            ]
